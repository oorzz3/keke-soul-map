...
- `style.css`：深藍星夜視覺、手機優先響應式、主卡 / 輔助卡視覺層級，以及詳情頁導覽、active 狀態與 route fallback 樣式。
- `app.js`：讀取 `KekeSoulData` 與 `KekeDetailPages`，渲染首頁總控台與命盤詳情頁，負責 active 狀態、詳情頁導覽與 route fallback。
...
- `data/detail-pages-data.js`：命盤詳情頁 mock / experiment / planning 資料，包含 `order`、`navLabel`、`icon`、`category`。
