...
function renderNameCalculationBoundary(boundary = {}) {
  ...
  const nextStep = "先完成 seed 資料表草案規格，再評估是否建立筆畫資料檔";
...
function getNumerologyInterpretationDisplay(display = {}) {
  ...
  return {
    ...
    version: meanings?.meta?.version || data?.numerologyInterpretation?.version || "v0.7.6",
...
function renderNumerologyCalculationBadge(displayData) {
  ...
  const label = status === "calculated" ? `calculated ${calculation.version || "v0.7.6"}` : "calculation fallback";
