...
- v0.6.2.1 只修正首頁生命靈數入口與今日數字節奏小卡的語意重複。
...
- 保留 lunar-javascript 實驗資料，不把農民曆升格成主功能。
...
版本：v0.5.1.6
  ...
  農民曆與神明生日仍維持 support 短卡 / anchor。
  不新增 `#/module/almanac` 或 `#/module/deity`。
  不改 `features/almanac-engine.js`、`features/deity-matcher.js` 或 `features/date-test-mode.js`。
...
## v0.5.1.4 support 短卡規則
  ...
  農民曆與神明生日仍是 dashboard support card / anchor，不進入 detail route。
  首頁農民曆只保留日期與一行提醒，不顯示大段安全線。
...
注意：
  ...
  本版不新增 #/module/almanac 或 #/module/deity。
  v0.5.1.4 首頁 Blueprint 視覺二修後，農民曆與神明生日維持短卡化 support card，不進入命盤核心主視覺。
  本版只文件化現況。
...
目前型態：
  ...
  v0.5.1.4 已配合首頁 Blueprint 視覺二修，農民曆只保留短卡摘要與 anchor
...
後續深化方向：
  ...
  v0.5.1.4 首頁只保留短卡化 support 摘要，不新增農民曆 detail route
...
## v0.5.x 深化順序建議
  ...
  v0.5.1.4 首頁 Blueprint 視覺二修（已啟動）
