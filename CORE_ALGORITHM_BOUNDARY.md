...
v0.7.0 建立總架構文件；v0.7.1 鎖定姓名學為第二核心候選；v0.7.3 整理姓名學 metadata 顯示；v0.7.4 新增 `NAME_STROKE_RULES_SPEC.md`，用文件先鎖定姓名學筆畫規則、資料表欄位、來源授權、fallback 與測試案例。本版不新增姓名學 calculator，不新增姓名學 meaning data，不改首頁 UI，也不將姓名學標示為 calculated。
